...
class CreateView(ft.View):
    ...
    def __init__(self, page: ft.Page, app_state: AppState):
        ...
        self.create_item_button = ft.ElevatedButton(
            'Crear Planta',
            color=SECONDARY_COLOR,
            bgcolor=PRIMARY_COLOR,
            on_click=self.create_item
        )
    ...
    async def create_item(self, e: ft.ControlEvent):
        self.create_item_button.disabled = True
        self.page.update()
        ...
        try:
            if not self.image_bytes:
                raise ValueError('Error: Debe cargar una imagen.')
            ...
            self.create_item_button.disabled = False
            self.page.update()
        ...
        try:
            ...
            response.raise_for_status()
            self.page.open(ft.SnackBar(content=ft.Text('Planta Agregada Exitosamente', color="white"), bgcolor=ft.Colors.GREEN_400, duration=1000))
